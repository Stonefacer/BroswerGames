    ...
    DoEvents: function(){
      for(i=0;i<this.Queue.length;i++){
        ...
        if (current.Delay <= 0) {
          ...
          if (Array.isArray(current.Handler)) {
...
function Init() {
  try {
    ...
    for (i = 0; i < StaticImages.length; i++) {
      ...
      StaticImages[i].Image.onload = function () { scope.loader(); };
    }
    for (i = 0; i < Selectors.length; i++) {
      ...
      Selectors[i].Image.onload = function () { scope.loader(); };
    }
    for (i = 0; i < Test.length; i++) {
